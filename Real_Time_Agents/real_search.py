"""
Tools and real search integrations for the Research Assistant.
Implements DuckDuckGo, SerpAPI, Tavily, and other utility tools.
"""
...
SEARCH_ENGINE = os.environ.get("SEARCH_ENGINE", "tavily").lower()
...
def search_duckduckgo(query: str, max_results: int = 5) -> str:
    try:
        ...
        return f"[DuckDuckGo - {len(results)} results for: '{query}']\n\n" + "\n---\n".join(results)
    except ImportError:
        return "Error: Run: pip install ddgs"
    except Exception as e:
        return f"DuckDuckGo error: {e}"

def search_serpapi(query: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        return f"[SerpAPI/Google - {len(formatted)} results for: '{query}']\n\n" + "\n---\n".join(formatted)
    except ImportError:
...
def search_tavily(query: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        header = f"[Tavily - {len(formatted)} results for: '{query}']\n"
        if answer:
            header += f"\nAI Answer: {answer}\n"
        return header + "\n---\n".join(formatted)
    except ImportError:
...
def read_file(path: str) -> str:
    ...
    try:
        ...
        if char_count > 50_000:
            ...
            return f"[FILE: {safe_path} | {char_count:,} chars - showing first 50,000]\n\n" + content + "\n\n[...file truncated - too large for single read]"
...
if __name__ == "__main__":
    print("Testing unified tools via real_search.py")
    ...
    query = input("\nEnter search query [default: 'Anthropic job requirements']: ").strip()
    if not query:
        query = "Anthropic job requirements"

    print(f"\n--- Testing search_web ({mode_str}) ---")
    search_results = search_web(query)
    print(search_results[:800])
    
    print("\n--- Testing read_file ---")
    print(read_file("real_search.py")[:100])
    
    print("\n--- Testing write_file ---")
    print(write_file("test_output.txt", search_results))
